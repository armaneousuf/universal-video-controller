...
const defaultSettings = {
  ...
  autoScrollEnabled: false,      // Reels/Shorts auto scroll
  stayVisible: false
};
...
function applyAudioBoost(vid, multiplier) {
  ...
  try {
    ...
    if (!vid.__uvc_audio_error) {
      console.warn("UVC Audio Booster configuration rejected (likely cross-origin CORS limitations):", err);
...
function triggerFeedScroll() {
  ...
  if (isInstagram) {
    ...
    if (vid) {
      ...
      if (scrollTarget) {
        scrollTarget.scrollBy({ top: scrollTarget.clientHeight, behavior: "smooth" });
        return;
      }
      ...
      const touchOpts = (y) => ({
        ...
        touches: [new Touch({ identifier: Date.now(), target: vid, clientX: touchX, clientY: y, radiusX: 2, radiusY: 2, rotationAngle: 0, force: 1 })],
        changedTouches: [new Touch({ identifier: Date.now(), target: vid, clientX: touchX, clientY: y, radiusX: 2, radiusY: 2, rotationAngle: 0, force: 1 })]
      });
